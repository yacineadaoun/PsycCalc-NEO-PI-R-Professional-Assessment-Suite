...
# --- Run scan only when the user asks (prevents full re-scan on every widget change) ---
img_bytes = img_file.getvalue()
key_bytes = key_file.getvalue() if key_file is not None else default_key_path.read_bytes()
cfg_sig = json.dumps(cfg.__dict__, sort_keys=True, default=str).encode("utf-8")
scan_sig = hashlib.sha256(img_bytes + key_bytes + cfg_sig).hexdigest()
...
with scan_col2:
    st.caption("Le scan ne se relance que quand tu cliques sur **Lancer le scan** (plus stable sur Streamlit Cloud).")

if run_scan or (st.session_state.scan_result is None) or (st.session_state.scan_sig != scan_sig):
    if not run_scan and st.session_state.scan_result is not None and st.session_state.scan_sig != scan_sig:
        st.warning("Paramètres ou fichiers changés → relance le scan.")
        st.stop()
    with st.spinner("Scan en cours..."):
        st.session_state.scan_result = scanner.scan_pil(pil_img, scoring_key)
...
st.subheader("Scores (résultat initial)")
st.json({"domain_scores": result.domain_scores, "facette_scores": result.facette_scores})
...
# Apply corrections and recompute (only when you click)
final_resp = dict(result.responses_final)
final_resp.update(corrections)

if corrections:
    st.info(f"{len(corrections)} correction(s) prêtes. Clique pour recalculer.")
    if st.button("✅ Appliquer corrections et recalculer", use_container_width=True):
        ...
        st.subheader("Scores (après corrections)")
        st.json({"protocol": proto2, "domain_scores": domain_scores, "facette_scores": facette_scores})
        ...
        bytes_download_button(
            "Télécharger responses_corrigées.csv",
            out.getvalue().encode("utf-8"),
            "responses_corrigees.csv",
            "text/csv",
        )
        ...
        bytes_download_button(
            ...
            json.dumps(
                {"protocol": proto2, "facette_scores": facette_scores, "domain_scores": domain_scores},
                ensure_ascii=False,
                indent=2,
            ).encode("utf-8"),
